...
func TestRunning() bool {
	_, err := CreateMutex("TempestDice")
	if err == nil {
		return false
	}

	s1, _ := syscall.UTF16PtrFromString("TempestDice 风暴已经在运作")
	s2, _ := syscall.UTF16PtrFromString("如果你想在Windows上打开多个风暴，请点“确定”，或加参数-m启动。\n如果只是打开UI界面，请在任务栏右下角的系统托盘区域找到风暴图标并右键，点“取消")
	ret := win.MessageBox(0, s2, s1, win.MB_YESNO|win.MB_ICONWARNING|win.MB_DEFBUTTON2)
	return ret != win.IDYES
}

func PortExistsWarn() {
	s1, _ := syscall.UTF16PtrFromString("TempestDice 启动失败")
...
func getAutoStart() *autostart.App {
	...
	if err == nil {
		...
		autostartName := fmt.Sprintf("TempestDice_%s", pathName)

		appStart := &autostart.App{
			Name:        autostartName,
			DisplayName: "风暴骰点核心 - 目录: " + pathName,
...
func onReady() {
	...
	systray.SetTitle("TempestDice Core")
	systray.SetTooltip("TempestDice Core " + ver)

	mOpen := systray.AddMenuItem("打开界面", "开启WebUI")
	mOpenExeDir := systray.AddMenuItem("打开风暴目录", "资源管理器访问程序所在目录")
	...
	go func() {
		_ = beeep.Notify("TempestDice", "我藏在托盘区域了，点我的小图标可以快速打开UI", "assets/information.png")
	}()
	...
	for {
		...
		case <-mAutoBoot.ClickedCh:
			if mAutoBoot.Checked() {
				...
				if err != nil {
					s1, _ := syscall.UTF16PtrFromString("TempestDice 临时目录错误")
...
func httpServe(e *echo.Echo, dm *dice.DiceManager, hideUI bool) {
	...
	go func() {
		...
		for {
			...
			runtime.LockOSThread()
			systray.SetTooltip("TempestDice Core " + ver + " #" + portStr)
	...
	for {
		...
		if err != nil {
			s1, _ := syscall.UTF16PtrFromString("TempestDice Core")
...
func tempDirWarn() {
	s1, _ := syscall.UTF16PtrFromString("TempestDice 临时目录错误")
	s2, _ := syscall.UTF16PtrFromString("你正在临时文件目录运行风暴，最可能的情况是没有解压而是直接双击运行！\n请先完整解压后再进行运行操作！\n按确定后将自动退出")
	win.MessageBox(0, s2, s1, win.MB_OK|win.MB_ICONERROR)
