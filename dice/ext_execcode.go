...
func ExecCodeJsInit(d *Dice, vm *goja.Runtime, ctx *MsgContext, msg *Message, cmdArgs *CmdArgs) {
	...
	_ = vm.Set("memberUnban", MemberUnban)
	_ = vm.Set("memberWholeBan", MemberWholeBan)
	_ = vm.Set("memberKick", MemberKick)
	_ = vm.Set("EditMessage", EditMessage)
	_ = vm.Set("RecallMessage", RecallMessage)
	_ = vm.Set("SendToGroupNotice", SendToGroupNotice)
	_ = vm.Set("SendLike", SendLike)
	_ = vm.Set("SetGroupAdmin", SetGroupAdmin)
	_ = vm.Set("SetGroupName", SetGroupName)
	_ = vm.Set("SetSelfLongNick", SetSelfLongNick)
...
func RegisterExecCodeCommands(d *Dice) {
	helpForExecCodeLua := ".execcode lua [代码] //运行Lua代码"
	helpForExecCodeJs := ".execcode js [代码] //运行JavaScript代码"
	helpForExecCodeDs := ".execcode ds [代码] //运行DiceScript代码"
	helpForExecCodeBrief := "`.ec/.execcode` 运行代码指令"
	helpForExecCode := helpForExecCodeBrief + "\n" + helpForExecCodeLua + "\n" + helpForExecCodeJs + "\n" + helpForExecCodeDs
	cmdExecCode := CmdItemInfo{
		...
		Solve: func(ctx *MsgContext, msg *Message, cmdArgs *CmdArgs) CmdExecuteResult {
			...
			case "lua":
				if cmdArgs.GetArgN(2) == "help" {
					ReplyToSender(ctx, msg, helpForExecCodeLua)
					return CmdExecuteResult{Matched: true, Solved: true}
				}
				//---------------------------替换容错-------------------------------------
			...
			case "ds", "dicescript":
				if cmdArgs.GetArgN(2) == "help" {
					ReplyToSender(ctx, msg, helpForExecCodeDs)
					return CmdExecuteResult{Matched: true, Solved: true}
				}
				code := strings.Join(cmdArgs.Args[1:], " ")
				vm := ds.NewVM()
			...
			case "js", "javascript":
				if cmdArgs.GetArgN(2) == "help" {
					ReplyToSender(ctx, msg, helpForExecCodeJs)
					return CmdExecuteResult{Matched: true, Solved: true}
				}
				code := strings.Join(cmdArgs.Args[1:], " ")
				vm := goja.New()
