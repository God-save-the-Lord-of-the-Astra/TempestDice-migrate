...
func (d *Dice) registerCoreCommands() {
	...
	cmdBlack := &CmdItemInfo{
		...
		Solve: func(ctx *MsgContext, msg *Message, cmdArgs *CmdArgs) CmdExecuteResult {
			cmdArgs.ChopPrefixToArgsWith("add", "rm", "del", "list", "show", "find", "trust", "import", "in", "out")
			...
			case "in":
				...
				var UID string
				if strings.ToLower(subval) == "p" || strings.ToLower(subval) == "g" {
					reBlackID := "^[1-9][0-9]{4,12}$"
					if matched, _ := regexp.MatchString(reBlackID, trdval); matched {
						if strings.ToLower(subval) == "p" {
							UID = fmt.Sprintf("%s%s", "QQ:", trdval)
						} else if strings.ToLower(subval) == "g" {
							UID = fmt.Sprintf("%s%s", "QQ-Group:", trdval)
						}
					} else if strings.ToLower(trdval) == "" {
						if !strings.HasPrefix(subval, "QQ-Group:") && !strings.HasPrefix(subval, "QQ:") {
							return CmdExecuteResult{Matched: true, Solved: true}
						} else {
							UID = subval
						}
				...
				v, exists := d.BanList.Map.Load(UID)
				if !exists {
					ReplyToSender(ctx, msg, fmt.Sprintf("所查询的<%s>情况：正常(0)", UID))
					return CmdExecuteResult{Matched: true, Solved: true}
				}

				var text = fmt.Sprintf("所查询的<%s>情况：", UID)
				switch v.Rank {
				case BanRankBanned:
					text += "禁止(-30)"
				case BanRankWarn:
			...
			case "out":
				...
				reBlackID := "^[1-9][0-9]{4,12}$"
				if strings.ToLower(subval) == "p" {
					for _, trdval := range cmdArgs.Args[3:] {
						if matched, _ := regexp.MatchString(reBlackID, trdval); matched {
							UIDList = append(UIDList, fmt.Sprintf("%s%s", "QQ:", trdval))
						}
					}
				} else if strings.ToLower(subval) == "g" {
					for _, trdval := range cmdArgs.Args[3:] {
						if matched, _ := regexp.MatchString(reBlackID, trdval); matched {
							UIDList = append(UIDList, fmt.Sprintf("%s%s", "QQ-Group:", trdval))
						}
					}
				} else if strings.HasPrefix(subval, "QQ:") || strings.HasPrefix(subval, "QQ-Group:") {
					for _, trdval := range cmdArgs.Args[1:] {
						if strings.HasPrefix(trdval, "QQ:") || strings.HasPrefix(trdval, "QQ-Group:") {
							UIDList = append(UIDList, trdval)
						}
					}
				} else {
					return CmdExecuteResult{Matched: true, Solved: true}
...
.master jsclear <插件ID> // 清除指定插件的存储，随后重载JS环境`
	...
	cmdMaster := &CmdItemInfo{
		...
		Solve: func(ctx *MsgContext, msg *Message, cmdArgs *CmdArgs) CmdExecuteResult {
			...
			case "reboot":
				...
				code := cmdArgs.GetArgN(2)
				if code == "" {
					updateCode = strconv.FormatInt(rand.Int64()%8999+1000, 10)
	...
	helpGroup := ".group ban <统一ID> <时长>// 禁言群成员\n" +
		".group kick <统一ID> // 踢出群成员"
	cmdGroup := &CmdItemInfo{
		Name:      "group",
		ShortHelp: helpGroup,
		Help:      "群管理指令:\n" + helpGroup,
		Solve: func(ctx *MsgContext, msg *Message, cmdArgs *CmdArgs) CmdExecuteResult {
			...
			switch val {
			case "ban":
				if strings.ToLower(cmdArgs.GetArgN(2)) == "whole" || strings.ToLower(cmdArgs.GetArgN(2)) == "all" {
					MemberWholeBan(ctx, GroupID, true)
					ReplyToSender(ctx, msg, "已禁言所有成员")
					return CmdExecuteResult{Matched: true, Solved: true}
				}
				UserID := cmdArgs.GetArgN(2)
				durationRaw := cmdArgs.GetArgN(3)
				if durationRaw == "" {
					return CmdExecuteResult{Matched: true, Solved: true}
				} else if strings.Contains(durationRaw, "d") || strings.Contains(durationRaw, "D") {
					if !strings.HasPrefix(durationRaw, "{") {
						durationRaw = fmt.Sprintf("%s%s%s", "{", durationRaw, "}")
					}
					durationRaw = ctx.EvalFString(durationRaw, nil).ToString()
					if ctx.EvalFString(durationRaw, nil).vm.Error != nil {
						return CmdExecuteResult{Matched: true, Solved: true}
					}
				}
				...
				ReplyToSender(ctx, msg, "已禁言")
				return CmdExecuteResult{Matched: true, Solved: true}
			case "kick":
				for _, uid := range readIDList(ctx, msg, cmdArgs) {
					MemberKick(ctx, GroupID, uid)
					ReplyToSender(ctx, msg, "已批量踢出")
				}
				return CmdExecuteResult{Matched: true, Solved: true}
			case "notice":
				content := strings.TrimSpace(strings.TrimPrefix(cmdArgs.RawArgs, cmdArgs.GetArgN(1)))
				if content == "" {
					return CmdExecuteResult{Matched: true, Solved: true}
				}
				SendToGroupNotice(ctx, GroupID, content)
				ReplyToSender(ctx, msg, "已发送群公告")
				return CmdExecuteResult{Matched: true, Solved: true}
			case "like":
				UID := cmdArgs.GetArgN(2)
				times, _ := strconv.Atoi(cmdArgs.GetArgN(3))
				Times := int(times)
				if !strings.Contains(UID, "QQ:") {
					if cmdArgs.GetArgN(3) == "" {
						times, _ := strconv.Atoi(cmdArgs.GetArgN(2))
						Times = int(times)
					} else {
						UID = fmt.Sprintf("%s%s", "QQ:", UID)
					}
				}
				if UID == "" {
				...
				SendLike(ctx, UID, Times)
				ReplyToSender(ctx, msg, "已点赞")
				return CmdExecuteResult{Matched: true, Solved: true}
			case "admin":
				UID := cmdArgs.GetArgN(2)
				SetGroupAdmin(ctx, GroupID, UID, true)
				ReplyToSender(ctx, msg, "已设为管理员")
				return CmdExecuteResult{Matched: true, Solved: true}
			case "unadmin":
				UID := cmdArgs.GetArgN(2)
				SetGroupAdmin(ctx, GroupID, UID, false)
				ReplyToSender(ctx, msg, "已取消管理员")
				return CmdExecuteResult{Matched: true, Solved: true}
			case "sharegroup":
				GID := cmdArgs.GetArgN(2)
				ShareGroup(ctx, GID)
				ReplyToSender(ctx, msg, "已分享群名片")
				return CmdExecuteResult{Matched: true, Solved: true}
			case "shareqq":
				UID := cmdArgs.GetArgN(2)
				SharePeer(ctx, GroupID, UID)
				ReplyToSender(ctx, msg, "已分享联系人名片")
				return CmdExecuteResult{Matched: true, Solved: true}
			case "groupname":
				...
				SetGroupName(ctx, GroupID, newName)
				ReplyToSender(ctx, msg, "已修改群名称")
				return CmdExecuteResult{Matched: true, Solved: true}
			default:
				return CmdExecuteResult{Matched: true, Solved: true, ShowHelp: true}
			}
