...
func (pa *PlatformAdapterGocq) MemberUnban(groupID string, userID string) {
	GID, _ := pa.mustExtractID(groupID)
	UID, _ := pa.mustExtractID(userID)
	type DetailParams struct {
		GroupID  int64  `json:"group_id"`
...
func (pa *PlatformAdapterGocq) MemberWholeBan(groupID string, enable bool) {
	GID, _ := pa.mustExtractID(groupID)
	type DetailParams struct {
		GroupID int64 `json:"group_id"`
...
func (pa *PlatformAdapterGocq) EditMessage(ctx *MsgContext, msgID string, message string) {
	pa.RecallMessage(ctx, msgID)
	if ctx.IsPrivate {
		pa.SendToPerson(ctx, ctx.Player.UserID, message, "")
	} else {
		pa.SendToGroup(ctx, ctx.Player.UserID, message, "")
	}
}

func (pa *PlatformAdapterGocq) RecallMessage(_ *MsgContext, msgID string) {
	msgIDint, _ := strconv.Atoi(msgID)
	msgIDInt := int32(msgIDint)
	type DetailParams struct {
...
// ---------------------------目前无法使用，不知道为什么-------------------------------------
func (pa *PlatformAdapterGocq) SendToGroupNotice(_ *MsgContext, groupID string, content string) {
	GID, _ := pa.mustExtractID(groupID)
	type DetailParams struct {
		GroupID int64  `json:"group_id"`
...
func (pa *PlatformAdapterGocq) SendLike(_ *MsgContext, userID string, times int) {
	UID, _ := pa.mustExtractID(userID)
	type DetailParams struct {
		UserID int64 `json:"user_id"`
...
func (pa *PlatformAdapterGocq) SetGroupAdmin(_ *MsgContext, groupID string, userID string, enable bool) {
	GID, _ := pa.mustExtractID(groupID)
	UID, _ := pa.mustExtractID(userID)
	type DetailParams struct {
		GroupID int64 `json:"group_id"`
...
func (pa *PlatformAdapterGocq) SetGroupName(_ *MsgContext, groupID string, groupName string) {
	GID, _ := pa.mustExtractID(groupID)
	type DetailParams struct {
		GroupID   int64  `json:"group_id"`
...
func (pa *PlatformAdapterGocq) SetSelfLongNick(_ *MsgContext, longNick string) {
	type DetailParams struct {
...
func (pa *PlatformAdapterGocq) SharePeer(_ *MsgContext, groupID string, userID string) {
	GID, _ := pa.mustExtractID(groupID)
	UID, _ := pa.mustExtractID(userID)
	type DetailParams struct {
		GroupID int64 `json:"group_id"`
...
func (pa *PlatformAdapterGocq) ShareGroup(_ *MsgContext, groupID string) {
	GID, _ := pa.mustExtractID(groupID)
	type DetailParams struct {
		GroupID int64 `json:"group_id"`
	}
