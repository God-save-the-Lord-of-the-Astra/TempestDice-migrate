...
var randSrc = rand.NewPCG(uint64(time.Now().UnixNano()), uint64(-time.Now().UnixNano()))

func RandStringBytesMaskImprSrcSB(n int) string {
	sb := strings.Builder{}
	sb.Grow(n)
	// A randSrc.Uint64() generates 64 random bits, enough for letterIdxMax characters!
	for i, cache, remain := n-1, randSrc.Uint64(), letterIdxMax; i >= 0; {
		if remain == 0 {
			cache, remain = randSrc.Uint64(), letterIdxMax
		}
